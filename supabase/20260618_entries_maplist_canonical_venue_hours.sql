-- Let rendered guide payloads inherit canonical venue hours.
--
-- entry_stops.hours remains a guide-specific override. When it is absent,
-- public entries_maplist now resolves hours from venue_hours first, then
-- venues.hours_note as a fallback note.
  ...
  select jsonb_agg(
    jsonb_strip_nulls(
      jsonb_build_object(
        ...
        'hours', coalesce(stop.hours, canonical_hours.hours, to_jsonb(nullif(venue.hours_note, '')))
      )
    )
