...
const diningStops = [
  ...
  stop({
    ...
    description: "Russ & Daughters is the classic appetizing shop for smoked fish, bagels, caviar, babka, and a century of Lower East Side food culture. Use the shop for takeaway or the cafe for a seated version.",
  ...
  stop({
    ...
    description: "Keens is the classic Midtown steakhouse to book for old New York atmosphere: clay pipes overhead, dark wood rooms, a famous mutton chop, and serious beef near the theaters. It is the one steakhouse anchor in this guide, so the category does not double up.",
  ...
  stop({
    ...
    description: "Sylvia's is a legendary Harlem soul-food restaurant known for fried chicken, ribs, collards, cornbread, and a dining room that has helped define Lenox Avenue for generations. Pair it with Apollo Theater or Studio Museum plans for a Harlem-focused food and culture route.",
  ...
  stop({
    ...
    description: "Nom Wah pairs Doyers Street history with old-school booths and dim sum staples, making it an easy Chinatown anchor for dumplings, buns, tea, and a neighborhood walk. It is strongest as a casual lunch or early dinner before continuing into the Lower East Side.",
  ...
  stop({
    ...
    description: "Lombardi's is the coal-oven pizza landmark tied to New York's first pizzeria story, best for splitting a whole pie before walking Nolita, Little Italy, or SoHo. It works here as food history with a real meal attached.",
...
const cheapEatStops = [
  ...
  stop({ id: "nyc-cheap-absolute-bagels", name: "Absolute Bagels", coordinates: [40.802546, -73.967807], description: "Absolute Bagels is an uptown breakfast classic for fresh bagels, spreads, and smoked-fish sandwiches near Columbia and the north end of Central Park. It gives the guide a true bagel destination outside the usual downtown route.", officialUrl: "https://www.google.com/maps/search/?api=1&query=Absolute%20Bagels%20New%20York%20City", photo: images.absoluteBagels, hours: "Google Maps listing and posted shop evidence provide the current-status source because Absolute Bagels has no official site; check that listing for day-specific hours.", price: "$", priceSource: "Google Maps / posted menu evidence", venueKind: "food_drink", foodServiceType: "bakery", cuisineTypes: ["bagels", "breakfast", "bakery"], attributeTags: ["budget", "breakfast", "counter_service", "uptown", "takeaway"], editorialUrls: ["https://ny.eater.com/maps/best-bagels-nyc", "https://www.theinfatuation.com/new-york/reviews/absolute-bagels"] }),
  ...
  stop({ id: "nyc-cheap-hh-bagels", name: "H&H Bagels", coordinates: [40.774433, -73.95448], description: "H&H Bagels adds another iconic New York bagel name to the list, with classic bagels, spreads, breakfast sandwiches, and multiple city locations. It keeps the cheap-eats guide from leaning too heavily on noodles and dumplings.", officialUrl: "https://www.hhbagels.com/", photo: images.hhBagels, hours: "Official H&H locations page lists store-specific hours for Upper East Side, Upper West Side, Moynihan, JFK, and LaGuardia branches.", price: "$", priceSource: "Official menu / Google Maps", venueKind: "food_drink", foodServiceType: "bakery", cuisineTypes: ["bagels", "breakfast", "bakery"], attributeTags: ["budget", "breakfast", "counter_service", "upper_east_side", "takeaway"], editorialUrls: ["https://www.cntraveler.com/story/best-bagels-new-york-city", "https://en.wikipedia.org/wiki/H%26H_Bagels"] }),
...
const hostelStops = [
  ...
  stop({ id: "nyc-hostel-west-side-ymca", name: "West Side YMCA", coordinates: [40.770826, -73.980451], description: "West Side YMCA is a bare-bones Central Park/Lincoln Center budget base with private-style rooms and shared-facility practicality rather than hostel-party energy. It belongs here because location can beat amenities for some travelers; read room and bathroom details carefully before booking.", officialUrl: "https://ymcanyc.org/locations/west-side-ymca/guest-rooms", bookingUrl: "https://ymcanyc.org/locations/west-side-ymca/guest-rooms", photo: images.westSideYmca, hours: "Official YMCA guest-rooms page and booking partners list date-specific room availability, facility access, and check-in policies.", price: "$$", priceSource: "Official booking site / Booking.com", venueKind: "lodging", lodgingType: "hostel", attributeTags: ["budget", "central", "quiet", "private_rooms", "family_friendly"], editorialUrls: ["https://www.booking.com/hostels/city/us/new-york.html", "https://www.google.com/travel/hotels/New%20York%20City?q=West%20Side%20YMCA%20guest%20rooms"] }),
...
const casualBarStops = [
  ...
  stop({ id: "nyc-bar-petes-tavern", name: "Pete's Tavern", coordinates: [40.736816, -73.986772], description: "Pete's Tavern is a Gramercy classic for an old bar, Italian-American comfort food, O. Henry lore, and an easy pint near Union Square. It gives the guide a reliable pub-style meal without feeling like a hotel lounge.", officialUrl: "https://www.petestavern.com/", photo: images.petesTavern, hours: "Official site posts daily bar and dining hours; verify holiday hours.", price: "$$", priceSource: "Official menu / Google Maps", venueKind: "nightlife", nightlifeType: "pub", attributeTags: ["historic", "casual", "food_available", "gramercy", "group_friendly"], editorialUrls: ["https://ny.eater.com/maps/classic-bars-nyc", "https://www.timeout.com/newyork/bars/petes-tavern"] }),
  stop({ id: "nyc-bar-julius", name: "Julius'", coordinates: [40.73345, -74.002987], description: "Julius' is one of New York's essential queer bars, known for the 1966 Sip-In, burgers, beer, and a West Village room that still feels casual and lived-in. It deserves a dedicated LGBTQ+ guide later, but it also fits this historic dive-bar route.", officialUrl: "https://juliusbarny.com/", photo: images.julius, hours: "Official Julius' site and Google Maps listing provide current daily bar hours and event/service updates.", price: "$$", priceSource: "Official/menu evidence / Google Maps", venueKind: "nightlife", nightlifeType: "dive_bar", attributeTags: ["lgbtq", "historic", "casual", "west_village", "food_available"], editorialUrls: ["https://www.nyclgbtsites.org/site/julius-bar/", "https://www.timeout.com/newyork/bars/julius"] }),
  stop({ id: "nyc-bar-stonewall", name: "The Stonewall Inn", coordinates: [40.733824, -74.002164], description: "The Stonewall Inn is both a working LGBTQ+ bar and the landmark tied to the 1969 uprising, with drag, karaoke, DJs, and Christopher Park next door. It belongs in nightlife because the history and current bar programming are inseparable.", officialUrl: "https://thestonewallinnnyc.com/", photo: images.stonewall, hours: "Official site posts bar/event hours; verify current schedule before going.", price: "$$", priceSource: "Official/event listings / Google Maps", venueKind: "nightlife", nightlifeType: "pub", musicGenres: ["dj", "drag", "karaoke"], attributeTags: ["lgbtq", "historic", "events", "west_village", "lively"], editorialUrls: ["https://www.nyclgbtsites.org/site/stonewall-inn-christopher-park/", "https://www.nps.gov/places/stonewall-inn.htm"] }),
...
const cocktailStops = [
  ...
  stop({ id: "nyc-cocktail-dante", name: "Dante", coordinates: [40.728774, -74.001839], description: "Dante is the Greenwich Village aperitivo bar for negronis, spritzes, Martinis, and Italian-leaning food in a room that works from afternoon into evening. It is a strong choice when the night should begin with a drink rather than end with one.", officialUrl: "https://www.dante-nyc.com/", photo: images.dante, hours: "Official site posts daily cafe/bar hours by location; verify the MacDougal Street schedule.", price: "$$$", priceSource: "Official menu / Google Maps", venueKind: "nightlife", nightlifeType: "cocktail_bar", attributeTags: ["aperitivo", "reservation_recommended", "west_village", "cocktails", "food_available"], editorialUrls: ["https://ny.eater.com/maps/best-cocktail-bars-nyc", "https://www.worlds50bestbars.com/the-list/dante.html"] }),
  ...
  stop({ id: "nyc-cocktail-king-cole", name: "King Cole Bar", coordinates: [40.761521, -73.974284], description: "King Cole Bar is the Midtown hotel-bar classic where the Maxfield Parrish mural, martinis, and Red Snapper lore do the heavy lifting. It belongs here for a polished pre-theater or Midtown drink; expect hotel pricing and a more formal rhythm.", officialUrl: "https://www.marriott.com/en-us/hotels/nycxr-the-st-regis-new-york/dining/", photo: images.kingCole, hours: "Official St. Regis dining page lists current King Cole Bar service hours, dress guidance, and holiday schedule changes.", price: "$$$$", priceSource: "Official hotel dining page / Google Maps", venueKind: "nightlife", nightlifeType: "cocktail_bar", attributeTags: ["hotel_bar", "historic", "midtown", "luxury", "cocktails"], editorialUrls: ["https://www.timeout.com/newyork/bars/king-cole-bar", "https://www.theinfatuation.com/new-york/reviews/king-cole-bar"] }),
];

const cultureStops = [
  stop({ id: "nyc-culture-met", name: "The Metropolitan Museum of Art", coordinates: [40.779437, -73.963244], description: "The Met is New York's major encyclopedic museum, spanning ancient art, European paintings, American rooms, fashion, arms and armor, and major temporary exhibitions beside Central Park. It is the strongest single museum anchor for a first culture pass.", officialUrl: "https://www.metmuseum.org/", photo: images.met, hours: "Official site posts museum hours, closed days, and exhibition-specific ticketing; verify before visiting.", venueKind: "culture", subcategory: "museum", attributeTags: ["museum", "art", "classic", "upper_east_side", "family_friendly"], editorialUrls: ["https://www.nyctourism.com/museums-galleries/the-metropolitan-museum-of-art/", "https://www.timeout.com/newyork/museums/metropolitan-museum-of-art"] }),
