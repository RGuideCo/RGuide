...
const SQL_PATH = path.join(ROOT, "supabase/editorial-guides.sql");
...
const exportResult = spawnSync(process.execPath, ["scripts/export-editorial-guides-sql.mjs"], {
  cwd: ROOT,
  encoding: "utf8",
  stdio: "pipe",
});
...
try {
  ...
  console.log(`Pushed editorial guides to Supabase. editorial_guides rows: ${rows[0]?.count ?? "unknown"}`);
} catch (error) {
